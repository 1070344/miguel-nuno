...
{
protected:	
	/*comprimento primeira visita*/void lengthFirstVisitRecursive(typename list < graphVertex <TV,TE> >::iterator itv, bitset <MAX_VERTICES> &taken, queue <TV> &q) const;
...
public:
	...
	/*comprimento*/queue <TV> lengthFirstVisit(const TV &vContent);
	/*distancia*/queue <TV> breadthFirstVisit(const TV &vContent);
	/*caminhos distintos*/queue < stack <TV> > distinctPaths(const TV &vOrigin,const TV &vDestination);	
	/*caminho mais curto entre dois locais*/bool dijkstrasAlgorithm(const TV &vContent, vector <int> &pathKeys, vector <TE> &dist);
	/*retornar o caminho mais curto*/queue <TV> getDijkstrasPath(int vKeyOrigin, vector <int> pathKeys);
};
